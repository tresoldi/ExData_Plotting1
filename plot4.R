...
par(mfrow=c(2, 2))
plot(pc.subset$Time, pc.subset$Global_active_power, xlab="", ylab="Global Active Power", type="l")
plot(pc.subset$Time, pc.subset$Voltage, xlab="datetime", ylab="Voltage", type="l")
plot(pc.subset$Time, pc.subset$Sub_metering_1, xlab="", ylab="Energy sub metering", type="l")
plot.xy(xy.coords(pc.subset$Time, pc.subset$Sub_metering_2), col="red",  type="l")
plot.xy(xy.coords(pc.subset$Time, pc.subset$Sub_metering_3), col="blue", type="l")
my.legend = c("Sub_metering_1", "Sub_metering_2","Sub_metering_3")
legend("topright", legend=my.legend, lty=c(1,1,1), col=c("black","red","blue"), bty="n")
plot(pc.subset$Time, pc.subset$Global_reactive_power, xlab="datetime", ylab="Global_reactive_power", type="l")
dev.off()
